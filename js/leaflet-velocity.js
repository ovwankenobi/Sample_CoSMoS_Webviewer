...
var Windy = function Windy(params) {
  ...
  var VELOCITY_SCALE = 1*(params.velocityScale || 0.005) * (Math.pow(window.devicePixelRatio, 1 / 3) || 1); // scale for wind velocity (completely arbitrary--this value looks nice)
  ...
  var dt_update = 3600.0
  var animation_time_step = 1200.0
  var next_wind_update_time;
  var wind_field_dt;
  var last_wind_update_time = 0.0
  var mapBounds;
  var wind_field_times = [];
  var wind_field_dt;
  var wind_field_last_time;
  var fw = 0.0
  var iblock0 = 0
  var iblock1 = 1
...
//  }
  ...
  var interpolate = function interpolate(λ, φ, grd) {
    if (!grd) return null;
    ...
    if (row = grd[fj]) {
      ...
      if (isValue(g00) && isValue(g10) && (row = grd[cj])) {
        var g01 = row[fi];
        var g11 = row[ci];
  ...
  var createField = function createField(columns, bounds, callback) {
     ...
     */
    function field(x, y) {
      var column = columns[Math.round(x/iref)];
      return column && column[Math.round(y/iref)] || NULL_WIND_VECTOR;
...
//	     }
  ...
  var interpolateField = function interpolateField(grids, bounds, extent, i1) {
    ...
    function interpolateColumn(x,iii) {
      ...
      for (var y = bounds.y; y <= bounds.yMax; y += iref) {
        ...
        if (coord) {
          ...
          if (isFinite(λ)) {
            ...
            var wind = interpolate(λ, φ, grids[i1]);
  ...
  var animate = function animate(bounds, field) {
    ...
    (function frame() {
      ...
      if (delta > FRAME_TIME) {
		...
		if (animation_time>next_wind_update_time) {
			// update columns
            iblock0 += 1
            iblock1 += 1
            last_wind_update_time = next_wind_update_time
            next_wind_update_time += wind_field_dt
            columns0 = Array.from(columns1)
			interpolateField(grids, bounds, mapBounds, iblock1);
        }

		if (animation_time>=next_update_time) {
        ...
        animation_time += animation_time_step

        if (animation_time > wind_field_last_time) {
			animation_time = 0.0
			iblock0 = 0
			iblock1 = 1
			next_update_time = 0.0
			next_wind_update_time = wind_field_dt
            interpolateField(grids, bounds, mapBounds, 0);
            columns0 = Array.from(columns1)
            interpolateField(grids, bounds, mapBounds, 1);
            last_wind_update_time = 0.0
            next_wind_update_time = wind_field_dt
        }
  ...
  var start = function start(bounds, width, height, extent) {
    ...
    // Compute columns 1
    interpolateField(grids, bounds, mapBounds, iblock0);
    columns0 = Array.from(columns1)
    columns = Array.from(columns0)

    // Compute winds
    interpolateColumns(grids, bounds, extent, fw)

    field = createField(columns, bounds)
